...
One-shot Startup
- To bring up the entire stack in one command (with optional data seeding and E2E tests):
- Run: `./scripts/start-stack.sh`
- Options:
  - `--seed` seeds Neo4j with initial data (default)
  - `--no-seed` skip seeding
  - `--e2e` run UI E2E tests after startup (requires Playwright deps on host)
