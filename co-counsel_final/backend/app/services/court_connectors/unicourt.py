...
class UniCourtConnector(CourtConnector):
    provider_id = "unicourt"
    ...
    def ensure_ready(self) -> None:
        if not self._base_url:
            raise ConnectorNotConfigured("UniCourt endpoint is not configured")
    ...
    async def _request_json(self, path: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        headers: Dict[str, str] = {"Accept": "application/json", "User-Agent": "CoCounsel-UniCourt-Connector/1.0"}
        if self._api_key:
    ...
    async def search(
        ...
        *,
        ...
        for idx, item in enumerate(self._payload_items(payload)):
            case_id = str(item.get("id") or item.get("case_id") or item.get("docket_id") or f"unicourt-{idx + 1}")
            caption = str(item.get("caption") or item.get("case_name") or item.get("title") or f"UniCourt result {idx + 1}")
            results.append(
                CourtSearchResult(
                    ...
                    caption=caption,
                    court=item.get("court") or "State Trial Court",
                    ...
                    metadata={
                        **item,
                        ...
                        "jurisdiction": jurisdiction or "state_trial",
    ...
    async def fetch_document(
        ...
        *,
        ...
        return CourtDocument(
            ...
            title="UniCourt Document",
            filed_date=None,
            source_url=source_url,
            content_type="application/pdf",
            metadata={
                "authority_band": "binding",
                "jurisdiction": "state_trial",
    ...
    async def fetch_calendar(
        ...
        *,
        ...
        for idx, item in enumerate(self._payload_items(payload)):
            title = str(item.get("title") or item.get("event") or item.get("description") or f"UniCourt event {idx + 1}")
            date_value = item.get("date") or item.get("event_date") or item.get("scheduled_at")
            events.append(
                {
                    "provider": self.provider_id,
                    "event_id": str(item.get("id") or f"unicourt-event-{idx + 1}"),
                    "title": title,
                    "date": date_value,
