...
- **Solution**: Adopted Co-Counsel production profile with automated ingestion, privilege detection, and timeline synthesis; deployed the onboarding workflow to capture ROI assumptions and coordinate rollout with case teams.
...
## Deployment Timeline
1. **Week 0** — Ran `scripts/install_tier.sh prod --dry-run` for security review, configured OTLP collector and Grafana dashboards using `infra/profiles/prod.env`.
...
## Next Steps
- Evaluate production scale-out via Helm for cross-border investigations (requires <2 hour SLA and federated Grafana access).
